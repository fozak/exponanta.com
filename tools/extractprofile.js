...
document.querySelectorAll('.dist-value').forEach(el => el.remove());
...
//remove after education
const education = document.getElementById('education');
if (education) {
  // Walk up to the section element
  let educationSection = education.closest('section');
  if (educationSection) {
    // Remove all following siblings
    while (educationSection.nextElementSibling) {
      educationSection.nextElementSibling.remove();
    }
  }
}
//
