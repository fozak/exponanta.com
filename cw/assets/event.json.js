//this was v1 of prompting engine


const TASK = 'analize event.json and suggest minimal changes (if needed) to efficienty fetch doctype=Event. My requirements are 1) to use my exisitng CW record and stucture 2) have efifient way of separating past event (from current date) from future events'
...
// IIEE AUTO-CONFIRM — paste in console, read the last line*/
(async () => {
  ...
  console.table([
    { test: 'future events',  count: future.target.data.length, pass: futureOk },
    { test: 'past events',    count: past.target.data.length,   pass: pastOk   },
  ])

  console.log(futureOk && pastOk ? '✅ ALL PASS' : '❌ FAILED')
})()